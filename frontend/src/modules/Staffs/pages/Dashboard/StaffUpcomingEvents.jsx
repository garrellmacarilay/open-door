...
function StaffUpcomingEvents({ upcomingEvents = [], onAddEvent, onDeleteEvent }) {
  ...
  const handleAddEventSubmit = async (e) => {
    ...
    if (result?.success) {
      ...
      // ✅ SAFETY CHECK: Ensure result.event exists (requires update in Hook)
      // If the hook doesn't return the event, we fetch the whole list again or warn the user.
      if (onAddEvent && result.data) { 
         onAddEvent(result.data);
      } else if (onAddEvent && result.event) {
