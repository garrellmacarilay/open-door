...
function AdminUpcomingAppointments({ upcomingEvents }) {
  ...
  // 1. Sync & Filter props on load
  useEffect(() => {
    // Only keep events that are pending, approved, or rescheduled
  ...
  const handleStatusChange = async (newStatus, actionName) => {
    ...
    try {
      ...
      // 2. Update Local List Logic
      if (!isValidStatus(newStatus)) {
        // CASE A: Status is Completed/Declined/Cancelled -> REMOVE from list
        setLocalEvents(prev => prev.filter(e => e.id !== selectedAppointment.id));
        
        // Close modal automatically since it's gone from the list
        setTimeout(() => handleCloseModal(), 500);
      } else {
        // CASE B: Status is Approved/Rescheduled -> UPDATE item in list (keep visible)
        setLocalEvents(prev => prev.map(event => {
          if (event.id === selectedAppointment.id) {
            return {
              ...event,
              details: { ...event.details, status: newStatus }
            };
          }
          return event;
        }));
      ...
      // 3. Update Modal UI (so user sees the change before it closes)
            ...
            return (  
              ...
              >
                ...
                <div className="flex items-center gap-2 mb-2">
                  <svg width="14" height="14" viewBox="0 0 16 14" fill="none"><path d="M1 5L8 1L15 5V12C15 12.2652 14.8946 12.5196 14.7071 12.7071C14.5196 12.8946 14.2652 13 14 13H2C1.73478 13 1.48043 12.8946 1.29289 12.7071C1.10536 12.5196 1 12.2652 1 12V5Z" stroke="#0059FF" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                  <span className="text-xs text-black" style={{ fontFamily: 'Inter' }}>{office || 'Unknown Office'}</span>
                </div>

                {/* Date Row */}
                <div className="flex items-center gap-2 mb-2">
                  <svg width="14" height="14" viewBox="0 0 14 16" fill="none"><path d="M11 3H3C1.89543 3 1 3.89543 1 5V13C1 14.1046 1.89543 15 3 15H11C12.1046 15 13 14.1046 13 13V5C13 3.89543 12.1046 3 11 3Z" fill="white"/><path d="M9 1V5M5 1V5M1 7H13" stroke="#360055" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
          ...
          <div className="bg-white rounded-[10px] shadow-[0px_4px_50px_10px_rgba(125,125,125,0.25)] w-full max-w-[500px] flex flex-col relative">
            ...
            <div className="bg-[#122141] rounded-t-[10px] h-[60px] flex items-center justify-between px-6 shrink-0">
              ...
              <button onClick={handleCloseModal} className="text-white hover:opacity-70 transition">
                <svg width="15" height="15" viewBox="0 0 24 24" fill="none"><path d="M18 6L6 18M6 6L18 18" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
              </button>
            </div>
              ...
              <div className="border border-[#BCBABA] rounded-[10px] p-4 space-y-3">
                ...
                <div className="flex items-start justify-between gap-4">
                  <div>
                    ...
                    <div className="ml-6">
                      <span className="text-sm text-black" style={{ fontFamily: "Inter" }}>{selectedAppointment.studentName}</span>
                    </div>
                  </div>
                ...
                <div>
                  ...
                  <div className="ml-6 text-sm text-black" style={{ fontFamily: "Inter" }}>{selectedAppointment.office}</div>
                </div>
                ...
                <div>
                  ...
                  <div className="ml-6 text-sm text-black" style={{ fontFamily: "Inter" }}>{selectedAppointment.serviceType}</div>
                </div>

                <div>
                  <div className="flex items-center gap-2 mb-1">
                    <svg width="14" height="16" viewBox="0 0 14 16" fill="none"><path d="M11 3H3C1.89543 3 1 3.89543 1 5V13C1 14.1046 1.89543 15 3 15H11C12.1046 15 13 14.1046 13 13V5C13 3.89543 12.1046 3 11 3Z" fill="white"/><path d="M9 1V5M5 1V5M1 7H13" stroke="#360055" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
                ...
                <div>
                  <div className="flex items-center gap-2 mb-2">
                    <svg width="16" height="16" viewBox="0 0 18 18" fill="none"><path d="M10.5 2.25H4.5C4.10218 2.25 3.72064 2.40804 3.43934 2.68934C3.15804 2.97064 3 3.35218 3 3.75V14.25C3 14.6478 3.15804 15.0294 3.43934 15.3107C3.72064 15.592 4.10218 15.75 4.5 15.75H13.5C13.8978 15.75 14.2794 15.592 14.5607 15.3107C14.842 15.0294 15 14.6478 15 14.25V6.75L10.5 2.25Z" stroke="#000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/><path d="M10.5 2.25V6.75H15" stroke="#000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"/></svg>
            ...
            <div className="flex gap-3 justify-end px-6 pb-6 shrink-0">
              ...
              {/* CASE 3: Status is invalid for this list (e.g. user just clicked Complete) */}
              {!isValidStatus(selectedAppointment.status) && (
                 <span className="text-xs text-gray-500 italic self-center">Updated</span>
              )}
